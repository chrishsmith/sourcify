...
1. **Batch Screening API (POST /api/denied-party/batch):**
   ...
   - Status determined by top score: ≥90 = 'match', ≥60 = 'potential_match', else 'clear'
...
2. **Batch Screening UI (Tab 2 of DeniedPartySearch):**
   - 4-stage workflow: idle → preview → processing → complete
...
3. **Batch Result Detail Modal:**
   ...
   - Match score display (color-coded: red ≥90, amber ≥70, gray <70)
...
**Story:** P3-006 - Build PGA requirements lookup
...
3. Run: npm run build
4. If build passes, update prd.json to set passes: true for story P3-006
5. Append learnings to progress.txt with format:
   ### Story P3-006 - 2026-01-19
   **What I Did:** [summary]
   **Patterns Found:** [patterns]
   **Gotchas:** [gotchas]
   **For Next Time:** [advice]
6. Commit: git add -A && git commit -m "feat(P3-006): [description]"
